...
# Feed Service - Spring Boot hot reload
docker_build(
  'feedservice',
  context='feedService',
  dockerfile='feedService/Dockerfile',
  live_update=[
    sync('feedService/target/classes', '/app/BOOT-INF/classes'),
  ]
)
...
k8s_yaml('k8s/secret/feedservice-postgres-secret.yaml')
k8s_yaml('k8s/namespaces/feedservice-ns.yaml')
k8s_yaml('k8s/feedservice/postgres-statefull.yaml')
k8s_yaml('k8s/feedservice/server-stateless.yaml')
k8s_yaml('k8s/feedservice/services.yaml')
